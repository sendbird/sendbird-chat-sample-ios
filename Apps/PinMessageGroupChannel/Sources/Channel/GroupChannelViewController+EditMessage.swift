...
extension GroupChannelViewController {
    ...
    private func presentEditUserMessageAlert(for message: UserMessage) {
        ...
        if channel.pinnedMessageIds?.contains(message.messageId) ?? false {
            alert.addAction(
                UIAlertAction(title: "Unpin", style: .default) { [weak self] _ in
                    self?.unpinMessage(messageId: message.messageId)
                }
            )
        }else{
            alert.addAction(
                UIAlertAction(title: "Pin", style: .default) { [weak self] _ in
                    self?.pinMessage(messageId: message.messageId)
                }
            )
        }
