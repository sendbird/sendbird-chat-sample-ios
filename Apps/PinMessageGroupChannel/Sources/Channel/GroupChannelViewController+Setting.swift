...
extension GroupChannelViewController {
    ...
    @objc func didTouchSettingButton() {
        ...
        if showPinnedMessages{
            actionSheet.addAction(
                UIAlertAction(title: "Show All messages", style: .default) { [weak self] _ in
                    self?.showPinnedMessages = false
                }
            )
        }else{
            actionSheet.addAction(
                UIAlertAction(title: "Show Pinned messages", style: .default) { [weak self] _ in
                    self?.showPinnedMessages = true
                }
            )
