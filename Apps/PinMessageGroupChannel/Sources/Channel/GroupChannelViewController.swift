...
class GroupChannelViewController: UIViewController {
    ...
    private lazy var tableView: UITableView = {
        ...
        tableView.register(PinMessageCell.self)
        tableView.register(PinFileCell.self)
    ...
    var showPinnedMessages:Bool = false{
        didSet{
...
extension GroupChannelViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if showPinnedMessages {
            return messageListUseCase.messages.filter({ channel.pinnedMessageIds?.contains($0.messageId) ?? true }).count
        }else{
            return messageListUseCase.messages.count
        }
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if showPinnedMessages {
            messages = messageListUseCase.messages.filter({channel.pinnedMessageIds?.contains($0.messageId) ?? true })
        }else{
            messages =  messageListUseCase.messages
        }
        let message = messages[indexPath.row]
        let isPinned = channel.pinnedMessageIds?.contains(message.messageId) ?? false
        if let fileMessage = message as? FileMessage {
            let cell: PinFileCell = tableView.dequeueReusableCell(for: indexPath)
            cell.configure(with: fileMessage, isPinned: isPinned)
            return cell
        } else {
            let cell: PinMessageCell = tableView.dequeueReusableCell(for: indexPath)
            cell.configure(with: message, isPinned: isPinned)
            return cell
        }
    }
...
//MARK: -PinUnpinMessage

extension GroupChannelViewController{
    func pinMessage(messageId:Int64){
        channel.pinMessage(messageId: messageId, completionHandler: {[weak self] error in
            if let error = error {
                self?.presentAlert(error: error)
                return
            }
    ...
    func unpinMessage(messageId:Int64){
        channel.unpinMessage(messageId: messageId, completionHandler: {[weak self] error in
            if let error = error{
                self?.presentAlert(error: error)
                return
            }
