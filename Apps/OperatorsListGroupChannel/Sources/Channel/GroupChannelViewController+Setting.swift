...
extension GroupChannelViewController {
    ...
    @objc func didTouchSettingButton() {
        ...
        actionSheet.addAction(
            UIAlertAction(title: "Operator List", style: .default) { [weak self] _ in
                self?.pushOperatorList()
            }
        )
    ...
    private func pushOperatorList() {
        let operatorListViewController = GroupChannelOperatorListViewController(channel: channel)
        
        navigationController?.pushViewController(operatorListViewController, animated: true)
    }
