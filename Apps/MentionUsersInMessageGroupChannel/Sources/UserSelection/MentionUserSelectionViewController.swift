...
public class MentionUserSelectionViewController: UIViewController {
    ...
    public init(channel: GroupChannel, didSelectUsers: @escaping DidSelectUserHandler) {
...
extension MentionUserSelectionViewController: UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let user = useCase.members[indexPath.row]
...
extension MentionUserSelectionViewController: UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        guard useCase.members.count > 0 && indexPath.row == useCase.members.count - 1 else { return }
...
extension MentionUserSelectionViewController: GroupChannelMemberListUseCaseDelegate {
    public func groupChannelMemberListUseCase(_ useCase: GroupChannelMemberListUseCase, didReceiveError error: SBError) {
        presentAlert(error: error)
    }
    
    public func groupChannelMemberListUseCase(_ useCase: GroupChannelMemberListUseCase, didUpdateMembers members: [Member]) {
