//
//  GroupChannelReadMembersListViewController.swift
...
class GroupChannelReadMembersListViewController: UITableViewController {
    
    private let channel: GroupChannel
    private let message: BaseMessage

    private lazy var useCase = MembersReadMessageUseCase(channel: channel)
    
    private var members: [Member] {
        return useCase.getReadMembers(for: message)
    }
    ...
    override func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        return members.count == 0 ? "No members read the message" : "Read By"
    }
